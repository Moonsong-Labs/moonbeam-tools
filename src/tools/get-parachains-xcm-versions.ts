...
import {prodParasKusama, prodParasKusamaCommon, prodParasPolkadot, prodParasPolkadotCommon} from "@polkadot/apps-config";
...
export async function getXCMVersion(provider: any): Promise<[string, string]> {
  ...
  let xcmpQueueVersion = provider.query.xcmpQueue && (await provider.query.xcmpQueue.palletVersion()) as any || "N/A";
  let xcmSafeVersion = provider.query.polkadotXcm && (await provider.query.polkadotXcm.safeXcmVersion()) as any || "N/A";
...
export async function getParaApi(network: "kusama" | "polkadot", id: string) {
  const prodParas = network == "kusama" ? [...prodParasKusama, ...prodParasKusamaCommon] : [...prodParasPolkadot, ...prodParasPolkadotCommon];
  const nodes =  Object.values(
    prodParas.find(
      (e) => e.paraId.toString() === id
    )?.providers || {}
...
const main = async () => {
  ...
  const network = relayRuntime.startsWith("kusama") ? "kusama" : relayRuntime.startsWith("polkadot") ? "polkadot" : null;
  if (!network) {
    console.log("Unknown network", relayRuntime);
    return;
  }
  const blockHash = (argv.at ? await api.rpc.chain.getBlockHash(argv.at || null) : await api.rpc.chain.getBlockHash()).toString();
  ...
  for (const id of Object.keys(paras)) {
    console.log(`Parachain ${id} ${paras[id].version[0] == "" ? "[DOWN]" : `[XCMQueue: ${paras[id].version[0]}, XCMSafe: ${paras[id].version[1]}]`} has ${paras[id].connections.length} connections`);
    console.log(`  ${paras[id].connections.map(c => `${c}[${paras[c].version[0]},${paras[c].version[1]}]`).join(", ")}`);
  }
